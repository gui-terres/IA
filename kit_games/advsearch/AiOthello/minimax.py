...
def minimax_move(state, max_depth: int, eval_func: Callable) -> Tuple[int, int]:
    ...
    _, best_move = max_value_1(state, float('-inf'), float('inf'), eval_func, max_depth, 0, current_player)
    return best_move

def max_value_1(state, alpha, beta, eval_func, max_depth, depth, player) -> Tuple[float, Tuple[int, int]]:
    ...
    for action in state.legal_moves():
        ...
        value, _ = min_value_1(successor, alpha, beta, eval_func, max_depth, depth + 1, player)
        if value > max_value:
            max_value = value
            best_move = action
...
def min_value_1(state, alpha, beta, eval_func, max_depth, depth, player) -> Tuple[float, Tuple[int, int]]:
    if state.is_terminal() or (max_depth != -1 and depth >= max_depth):
        return eval_func(state, player), None
    ...
    for action in state.legal_moves():
        successor = state.next_state(action)
        value, _ = max_value_1(successor, alpha, beta, eval_func, max_depth, depth + 1, player)
        if value < min_value:
            min_value = value
            best_move = action
